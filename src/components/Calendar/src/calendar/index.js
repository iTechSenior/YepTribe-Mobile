...
class Calendar extends Component {
  ...
  static propTypes = {
    ...
    /** Show calendar weekly if true */
    weeklyShow: PropTypes.bool,
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      selectedDate: xdateToData(XDate()),
    }
  ...
  render() {
    ...
    const allDays = dateutils.page(this.state.currentMonth, this.props.firstDay)
    const firstDay = this.state.selectedDate.day - this.state.selectedDate.dayOfWeek
    const index = firstDay < 0 ? 0 : allDays.findIndex(item => item.getDate() == firstDay)
    const weekDays = allDays.slice(index, index + 7)

    const weeks = []
    if (this.props.weeklyShow) {
      weeks.push(this.renderWeek(weekDays, weeks.length))
    } else {
      while (allDays.length) {
        weeks.push(this.renderWeek(allDays.splice(0, 7), weeks.length))
      }
    }
